...
function ShowTable() {
    ...
    const updateSceneName = (id, newName)=>{
        ...
        const updatedTransitions = transitions.map((transition)=>{
            if (transition.from === scenes.find((scene)=>scene.id === id)?.name) {
                return {
                    ...transition,
                    from: newName
                };
            }
            if (transition.to === scenes.find((scene)=>scene.id === id)?.name) {
                return {
                    ...transition,
                    to: newName
                };
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__3a125b._.js.map